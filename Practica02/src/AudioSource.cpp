...
void CAudioSource::SetPitch(float _pitch)                     { al_call(alSourcef(m_audioSourceBuffer, AL_PITCH, _pitch)); }
void CAudioSource::SetGain(float _gain)                       { al_call(alSourcef(m_audioSourceBuffer, AL_GAIN, _gain)); }
void CAudioSource::SetLooping(bool _loop)                     { al_call(alSourcei(m_audioSourceBuffer, AL_LOOPING, _loop ? 1 : 0)); }
void CAudioSource::SetPosition(float _x, float _y, float _z)  { al_call(alSource3f(m_audioSourceBuffer, AL_POSITION, _x, _y, _z)); }
void CAudioSource::SetVelocity(float _x, float _y, float _z)  { al_call(alSource3f(m_audioSourceBuffer, AL_VELOCITY, _x, _y, _z)); }

void CAudioSource::Play()   { al_call(alSourcePlay(m_audioSourceBuffer)); }
void CAudioSource::Stop()   { al_call(alSourceStop(m_audioSourceBuffer)); }
void CAudioSource::Pause()  { al_call(alSourcePause(m_audioSourceBuffer)); }
